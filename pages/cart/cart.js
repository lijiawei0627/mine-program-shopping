...
1 获取用户的收货地址
  1 绑定点击事件
  2 调用小程序内置 api  获取用户的收货地址  wx.chooseAddress（走不通）

  2 获取 用户 对小程序 所授予 获取地址的  权限 状态 scope
    1 假设 用户 点击获取收货地址的提示框 确定  authSetting对象中的scope.address 
      scope 值 true 直接调用 获取收货地址
    2 假设 用户 从来没有调用过 收货地址的api 
      scope undefined 直接调用 获取收货地址
    3 假设 用户 点击获取收货地址的提示框 取消   
      scope 值 false 此时无法再直接调用获取收货地址
  ...
  setCart(cart) {
    ...
    cart.forEach(v => {
      if (v.checked) {
        ...
        // 如果有一个商品未被选中，则设置allChecked为false
        allChecked = false;
      }
    })
    // 判断数组是否为空
